...
;;;; layer is rebuilt from revision parts.  (Deferred: find-in-page, mouse, regex.)
...
fetches and renders the target.  Return (values WINDOW FOCUS OPEN)."
  (let* ((cur (list base-url))
    ...
    (labels ((echo (m) (setf (static-text-text status) m) (invalidate status))
      ...
      (setf (hv-on-link doc)
            (lambda (href)
              (let ((target (%resolve-url (first cur) href)) (fetch *url-fetch-fn*))
                (if (and fetch target)
                    (progn                                   ; fetch off the UI thread; render when it lands
                      (echo (format nil " fetching ~a … " target))
                      (run-async (lambda () (funcall fetch target))
                                 :then (lambda (h) (if h (render h target)
                                                       (echo (format nil " could not fetch ~a " target))))
                                 :on-error (lambda (e) (echo (format nil " could not fetch ~a (~a) " target e)))
                                 :label "html-fetch"))
