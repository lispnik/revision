...
(defvar *ct-lock* (sb-thread:make-mutex :name "revision-calltree"))
...
(defun %ct-unwatch (sym)
  (when (member sym *ct-watched*)
    (ignore-errors (sb-int:unencapsulate sym 'revision-calltree))
...
;;; --- stepping / break (through the REPL + revision's debugger) -------------------

(defun do-step ()
  "Evaluate a form under the single-stepper in the REPL; each step surfaces in
revision's debugger (Step-next / Step-into / Step-out / Continue restarts)."
