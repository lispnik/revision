  ...
  (let* ((win (make-instance 'editor-window
    ...
    (let ((te (find-view win 'edit)))
      (if (and path (probe-file path))
          (te-load te path)
          (te-set-text te (format nil ";; revision scratch buffer~%;; type freely — Shift+arrows select, C-c/C-x/C-v copy/cut/paste, C-z/C-y undo/redo.~%~%(defun hello (name)~%  (format t \"hello, ~~a!~~%\" name))~%")))
