...
the cross-thread debugger (HANDLER-BIND keeps the stack live for the restart)."
  (let ((out (make-instance 'repl-stream :win win)) (msgs '()) (new-pkg nil))
    (let ((*standard-output* out) (*error-output* out) (*trace-output* out)
          ...
          ;; route break / single-step (any non-error debugger entry) to revision's
      ...
      (restart-case
          (handler-bind ((sb-ext:step-condition (lambda (c) (%repl-debug win c)))   ; single-stepper -> revision debugger
...
revision's evaluator and an embedded backend."
  (if (not *repl-time*)
  ...
  (let* ((win  (make-instance 'repl-window
    ...
    (let ((tr (find-view win 'transcript)))
      (scrollback-append tr
                         (format nil "revision REPL — ~a ~a~%evaluation runs on a background worker; the UI stays live (output streams in).~%~%"
